...
class GoogleAppsSaSheetsRowClearV1
  ...
  def execute()
    ...
    row = @parameters["row_number"]
    puts "Clearing the values from row #{} '#{@parameters["sheet_id"]}:#{@parameters["worksheet_name"]}'" if @enable_debug_logging
    result = client.execute(
      :api_method => method,
      :parameters => {
        'spreadsheetId' => @parameters['sheet_id'],
        'range' => "#{@parameters["worksheet_name"]}!A#{row}:ZZ#{row}"
      },
