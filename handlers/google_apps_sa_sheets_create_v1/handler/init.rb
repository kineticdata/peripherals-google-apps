...
class GoogleAppsSaSheetsCreateV1
  ...
  def execute()
    ...
    puts "Creating a new spreadsheet with the name '#{@parameters["name"]}'" if @enable_debug_logging
    ...
    if result.response.status.to_s != '200'
      ...
      spreadsheet_id = JSON.parse(result.response.body)['spreadsheetId']
    end

    # Return the results
    return <<-RESULTS
<results>
  <result name="Spreadsheet Id">#{spreadsheet_id}</result>
</results>
    RESULTS
  end
